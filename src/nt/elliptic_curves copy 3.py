...
class ConductorRecord:
    ...
    full_inclusion: bool = (
        False  # whether to include this conductor's curves in the final count (e.g. for SS, we only want to include the f=1 curves)
    )
...
class IsogenyClass:
    ...
    def field(self) -> ImaginaryQuadraticField:
        return (
            ImaginaryQuadraticField(self.D_pi)
            if self.D_pi < 0
            else QuaternionAlgebra(self.p)
        )
    ...
    @cached_property
    def size(self) -> int:
        return self.__N_t if self.__N_t is not None else self.field.Hf(self.f_pi)
    ...
    def _torsion_subgroup(
        self, f: int, N: int, compute_full: bool = False
    ) -> tuple[int, int]:
    ...
    def _compute_torsion_record(self, N: int, flatten: bool = True) -> TorsionRecord:
        ...
        if self.n_pts % N != 0:
            return TorsionRecord(
                n_curves=0, n_points=0, value=Fraction(0), conductor_levels=[]
            )
